...
var vtex_cpFunctions=
{
	...
	exec:function(productShelf)
	{
		...
		productShelf.find(vtex_cpFunctions.options.productsLi).each(function(){
			...
			if(!(skuArrayLength==1 && vtex_cpFunctions.options.linkAdjust(skuArray[0].split(";").pop()).trim()==($this.find(".vtex-cpProductLink:first").attr("href")||"").trim()))
                ...
                else if(jqXHR.status!=0)
                {
                    switch(action)
                    {
                        case 1:
                            ...
                            break;
                        case 2:
                            vtex_cpFunctions.mouseActions2(liElem, skuId, span, data, link);
                            break;
                    }
