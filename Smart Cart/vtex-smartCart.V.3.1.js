...
jQuery.fn.smartCart=function(opts)
{
	...
	var fns=
	{
		...
		productSuccess:function(data,url,btnOverlayE)
		{
			...
			options.formatSkuPopup(skuOptions).addClass("freeContent vtexsc-selectSku").vtexPopUp2();
			...
			if(chks.length>0)
			{
				...
				if(skuSpecifications.specifications>1)
					fns.multipleSkuConfigCheckbox(skuOptions, skuSpecifications, chks);
				else
		...
		multipleSkuConfigCheckbox:function(skusHtml, skuSpecifications, chks)
		{
			if(skuSpecifications.specifications>2)
			...
			chks.bind("change",function(){
				...
				ndx=$t.attr("id").split("_").pop();
				// $t.siblings("input").attr("checked",false);
