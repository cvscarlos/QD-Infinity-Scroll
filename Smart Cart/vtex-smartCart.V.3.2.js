...
jQuery.fn.smartCart=function(opts)
{
    ...
    var defaults=
	{
		...
		formatSkuPopup:function($skuList,$productImage)
		{
			var wrap,listWrap,btn,prodImg,skusWrap;
	...
	var fns=
	{
		...
		multiplesSkus:function(btn)
		{
			btn.bind("click",function(){
				var url,btnOverlayE,prodImgElem;
				...
				jQuery.ajax({
					url:url,
					success:function(data){fns.productSuccess(data,url,btnOverlayE,prodImgElem);},
					error:function()
					{
		...
		productSuccess:function(data,url,btnOverlayE,prodImgElem)
		{
			$data=jQuery(data);
			...
			options.formatSkuPopup(skuOptions,prodImgElem).addClass("freeContent vtexsc-selectSku").vtexPopUp2();
			...
			if(chks.length>0)
			{
				...
				if(skuSpecifications.specifications>1)
					fns.multipleSkuConfigCheckbox(skuOptions, skuSpecifications, chks, prodImgElem);
				else
		...
		multipleSkuConfigCheckbox:function(skusHtml, skuSpecifications, chks, prodImgElem)
		{
			if(skuSpecifications.specifications>2)
					...
					if(typeof skuSpecifications.skus[skuNdx] == "object" && $t.is(":checked") && t.is(":checked"))
					{
						...
						jQuery.ajax({
							...
							"success":function(data, textStatus, jqXHR)
							{
								...
								{
									imgs=fns.getImageUrl(data[0],options.skuThumbId);
